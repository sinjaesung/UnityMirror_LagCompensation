...
public class ServerCube : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            Debug.Log("ServerCube Time.time >= lastSendTime + sendInterva Send" + Time.time + ">=" + lastSendTime + sendInterval);
        ...
        {
            Debug.Log("ServerCube NetworkTime.localTime >= lastCaptureTime + lagCompensationSettings.captureInterval"
                + NetworkTime.localTime + ">="+ lastCaptureTime + lagCompensationSettings.captureInterval);
    ...
    {
        Debug.Log("ServerCube>> Send" + position);
        ...
        if (!drop)
        {
            //simulate scrable (Random.Next is < max, so +1)
            ...
            Debug.Log("ServerCube>> Send drop packet store queue insert>>" + deliveryTime);
        }
    }
    ...
    void Flush()
    {
        ...
        for(int i=0; i<queue.Count; ++i)
        {
            ...
            Debug.Log(i + "ServerCube Flush Queue action>>" + deliveryTime);
            if (NetworkTime.localTime >= deliveryTime)
            {
                Debug.Log("ServerCube Flush NetworkTime.localTime >= deliveryTime client Message");
    ...
    void Capture()
    {
        Debug.Log("ServerCube Capture>>" + NetworkTime.localTime + "," + transform.position + "," + col.size);
        //capture current state
        Capture2D capture = new Capture2D(NetworkTime.localTime, transform.position, col.size);

        Debug.Log("ServerCube Capture Insert into history>>");
        //insert into history
    ...
    public bool CmdClicked(Vector2 position)
    {
        ...
        Debug.Log($"ServerCube CmdClicked estimatedTime>>rtt:{rtt},estimatedTime:{estimatedTime}");
        // compare estimated time with actual client time for debugging
        double error = Math.Abs(estimatedTime - client.localTimeline);
        Debug.Log($"ServerCube CmdClicked: serverTime={NetworkTime.localTime:F3} clientTime={client.localTimeline:F3} estimatedTime={estimatedTime:F3} estimationError={error:F3} position={position}");
        ...
        {
            Debug.Log($"ServerCube CmdClicked LagCompensation.Sample resultBefore:{resultBefore},resultAfter:{resultAfter}");
