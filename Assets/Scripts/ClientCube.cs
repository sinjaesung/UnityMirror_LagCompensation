...
public class ClientCube : MonoBehaviour
{
    ...
    ExponentialMovingAverage deliveryTimeEma; //average delivery tiem(standard deviation gives average jitter)
    ...
    public void OnMessage(Snapshot3D snap)
    {
        ...
        Debug.Log("ClientCube>OnMessage>> SnapshotInterpolation.InsertAndAdjust");
        //insert into the buffer & initilaize / adjust / catchup
        ...
        if(snapshots.Count > 0)
        {
            ...
            if (interpolate)
            {
                ...
                SnapshotInterpolation.Step(
                    snapshots,
                    //accmulate delta is Time.unscaledDeltaTime normally,
                ...
                Debug.Log("ClientCube> SnapshotInterpolation Step from,to Snapshots" + fromSnapshot + "->" + toSnapshot);
                //interpolate & apply
    ...
    {
        //send the command.
        //only x coordinate matters for this simple examples.
    ...
    private void OnGUI()
    {
        ...
        //catchup is diaplayed as color state in Update() already.
